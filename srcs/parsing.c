...
/*   Updated: 2021/09/22 14:01:45 by mqueguin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	recover_playerpos_and_check_collect_exit(t_game *game)
{
	...
	while (game->map[++y])
	{
		...
		while (game->map[y][++x])
		{
			...
			{
				free_tab2d(game->map);
				printf("Error\nThe map contains a %c and it is not a valid character\n", game->map[y][x]);
				return (0);
			}
		}
	}
	...
	{
		free_tab2d(game->map);
		printf("Error\nThe map is not complete you need at least one collectible, one player and one exit\n");
		return (0);
	}
	return (1);
}
